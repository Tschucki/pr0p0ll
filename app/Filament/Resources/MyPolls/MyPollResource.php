...
namespace App\Filament\Resources\MyPolls;
...
use App\Filament\Resources\MyPolls\Pages\CreateMyPoll;
use App\Filament\Resources\MyPolls\Pages\EditMyPoll;
use App\Filament\Resources\MyPolls\Pages\ListMyPolls;
use App\Filament\Resources\MyPolls\Pages\ViewMyPoll;
...
class MyPollResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                TextColumn::make('status')->icon('heroicon-o-cog')->label('Status')->state(function (MyPoll $myPoll) {
                    if ($myPoll->hasEnded()) {
                        return 'Beendet';
                    }
            ...
            ->recordActions([
                Action::make('results')->visible(fn (MyPoll $poll) => $poll->visible_to_public === true)->button()->label('Ergebnisse ansehen')->url(fn (MyPoll $poll) => route('filament.pr0p0ll.resources.umfragen.results', ['record' => $poll])),
