...
class SendPollDeclinedEmailNotification implements ShouldQueue
{
    ...
    {
        if ($this->user->email && in_array('mail', $this->user->getNotificationRoutesForType(NotificationType::where('identifier', \App\Enums\NotificationType::POLLDECLINED)->first()), true)) {
